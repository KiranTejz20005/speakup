...
import { DEFAULT_PREPARATION_SECONDS, getCaptureUploadConfig } from "@/lib/practiceCapture";
...
export default function JamPractice() {
  ...
  }, [stage, remaining, sessionLength, recorder]);

  useEffect(() => {
    if (stage !== "speaking" || recorder.status === "paused") return;
    if (sessionSeconds === 0) {
      recorder.stop();
      setStage("finishing");
      return;
    }
    const id = window.setTimeout(() => setSessionSeconds(value => value - 1), 1000);
    return () => window.clearTimeout(id);
  }, [stage, sessionSeconds, recorder]);
  ...
  const begin = () => {
    ...
    if (!preparationSeconds) recorder.start();
  };
